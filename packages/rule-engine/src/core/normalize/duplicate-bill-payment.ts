...
const DuplicateBillPaymentNormalizationSchema = z
    .preprocess(
        (raw) => raw ?? {},
        z
            .object({
            ...
            .transform((data) => ({
                ...
                refNumber: (data.rawData as any)?.PaymentRefNum?.toString() || null,
                parsedRaw: data.rawData,
            }))
    );

export type NormalizedDuplicateBillPayment = z.infer<typeof DuplicateBillPaymentNormalizationSchema>;
...
export function normalizePayments(rawRecords: any[]): NormalizationResult {
    const payments: NormalizedDuplicateBillPayment[] = [];
    const unscannable: any[] = [];

    rawRecords.forEach((raw) => {
        const result = DuplicateBillPaymentNormalizationSchema.safeParse(raw);
        if (result.success) {
