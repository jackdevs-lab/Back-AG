...
export async function formatReport(
    ...
    const displayItems: ReportItem[] = reportData.findingsForDisplay.map((f: any) => {
        ...
        return {
            ...
            deepLink: `https://app.qbo.intuit.com/app/creditmemo?realmId=${ctx.realmId}&txnId=${f.metadata.qbId}`
        };
    });
    ...
    const errorItems: ReportItem[] = normErrors.map((err: any) => ({
        ...
        deepLink: err.qbId ? `https://app.qbo.intuit.com/app/creditmemo?realmId=${ctx.realmId}&txnId=${err.qbId}` : undefined
    }));
