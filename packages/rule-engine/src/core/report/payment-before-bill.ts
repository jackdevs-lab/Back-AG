    ...
    const report = formatStandardReport({
        ...
        items: findingsForDisplay.map((f: any) => {
            ...
            return {
                ...
                deepLink: [
                    `https://app.qbo.intuit.com/app/billpayment?realmId=${realmId}&txnId=${f.metadata.paymentId}`,
                    `https://app.qbo.intuit.com/app/bill?realmId=${realmId}&txnId=${f.metadata.billId}`
                ]
            };
        }),
