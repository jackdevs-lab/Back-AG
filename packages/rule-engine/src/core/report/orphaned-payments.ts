...
export async function formatReport(reportData: any, ctx: RuleContext, normErrors: any[]): Promise<string> {
    ...
    const displayItems: ReportItem[] = reportData.findingsForDisplay.map((f: any) => {
        ...
        const formattedAmount = Number(f.amount).toFixed(2);
        const formattedDate = f.date.toISOString().split('T')[0];

        return {
            id: f.metadata.qbId,
            label: `${customerName} - Payment ${f.metadata.qbId}`,
            details: `Payment for $${formattedAmount} on ${formattedDate} is not linked to any invoice.`,
    ...
    return formatStandardReport({
        title: 'Orphaned Customer Payments',
        items: displayItems,
